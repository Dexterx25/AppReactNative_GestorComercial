...
const {values, isSubmitting, setFieldValue, handleSubmit, errors} = useFormik({
  ...
  onSubmit: values =>{
    console.log(values)
      ...
      if(!values.description||!values.typeActivity||!values.OrganizationID||!values.date||!values.time) 
  ...
  validate:  values =>{
     ...
     if(values.description.length<10||!values.description) 
   {
    ...
    if(values.OrganizationID.length<10||!values.OrganizationID){
       errors.OrganizationID = "Invalid Date";
      }
