...
const {values, isSubmitting, setFieldValue, handleSubmit, errors} = useFormik({
  ...
  onSubmit: values =>{
    console.log(values)
      ...
      if(!values.namePerson||!values.phonePerson||!values.emailPerson||!values.OrganizationID||!values.typeidentity) 
  ...
  validate:  values =>{
     ...
     if(values.phonePerson.length < 10 || values.phonePerson.length >10 ) 
   {
    ...
    if(values.OrganizationID.length < 10|| values.OrganizationID.length > 10 ){
       errors.OrganizationID = "Invalid Identity";
      }  
  ...
  // createHomeStak = () =>
   ...
   <View style={styles.container}>
              ...
              <Form style={styles.nameNavigator}>
                 <Text style={styles.welcome}>Organization Registration</Text>
                  ...
                  <Item error={errors.OrganizationID ? true : false}>
                    <Label >Identity:</Label>  
                      <Input value={values.OrganizationID} onChangeText={text => setFieldValue('OrganizationID', text)}/>
                      <Text>{errors.OrganizationID ? errors.OrganizationID : ''}</Text>
                    </Item>
