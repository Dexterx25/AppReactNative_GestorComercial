...
function BusinePerson(){
    ...
    const {values, isSubmitting, setFieldValue, handleSubmit, errors} = useFormik({
      ...
      onSubmit: values =>{
        if(!values.OrganizationID||!values.nameBusine||!values.description||!values.valuebusine||!values.dateClose||!values.stateBusine) 
        {
          ...
          if(!values.nameBusine) 
        {
         ...
         if(values.OrganizationID.length >10){
     ...
     <Form style={styles.nameNavigator}>
       <Text style={styles.welcome}>The Busine Register</Text>
         ...
         <Item  error={errors.OrganizationID ? true : false}>
           <Label>your OrganizationID :</Label>         
